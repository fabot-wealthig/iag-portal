...
| # | Command | Expected |
| --- | --- | --- |
| 1 | MCP `supabase-iag` → `list_edge_functions` | `iag-admin-api`, `ACTIVE`, `verify_jwt: false`, version **17** (v: 2026-09-02) |
...
| 4 | action count — see command below | `32` table entries + 1 direct = **33** actions (v: 2026-09-02) |
...
**Action count (#4)** — `$p='C:\iag-edge-functions\supabase\functions\iag-admin-api\router\dispatch.ts'`,
then `(Select-String -Path $p -Pattern '^\s+"[a-z_]+":' | Measure-Object).Count`. Expected `32` =
`PUBLIC_HANDLERS` (5) + `AUTH_HANDLERS` (27), plus `admin_login` (dispatched directly in `index.ts`,
in neither table) = **33 actions total**.

**Anon probe (#8)** — the anon key must see nothing. GET each of the 12 tables (LIVE STATE →
Database) at `https://gqznnyccridnpipjipeq.supabase.co/rest/v1/<table>?select=*` with the anon key
as BOTH `apikey` and `Authorization: Bearer`, plus `Prefer: count=exact`; expect
`Content-Range: */0` on every one (v: 2026-09-02). It MUST be a GET with the count header —
`curl -I` answers `*/*` on locked and open tables alike. GOTCHA #7.

## SECURITY INVARIANTS

These four are FINAL. Re-check them on any table, policy, handler, or function change. An
invariant change is a headline, never a quiet edit. **(Confirmed UNCHANGED at chat-5 wrap-up.)**
...
- **#5/#13/#15** Backend deploys run `scripts/deploy-function.sh` (multipart upload to the Supabase
  Management API): from PowerShell as `& "$HOME\scoop\apps\git\current\usr\bin\bash.exe"
  scripts/deploy-function.sh` (a bare `bash` there is the WSL relay stub, #15), from Git Bash as
  `bash scripts/deploy-function.sh`. NEVER the `supabase` CLI (its login belongs to VFO); MCP
...
| `docs/flows/client-payment-request.md` | End-to-end client fee: request form → row + customer + token + draft → `/pay` → Stripe Checkout. |
...
- **Frontend shape (v: 2026-09-02):** 6 routes — `/` Landing, `/login`, `/portal` (the whole
  ...
  `set-password`, `payout-setup`, `pay` — or it serves a real 404. Both token pages go live at the
  NEXT `npm run deploy`; emailed links point at production, so until then they 404. Styling is
...
- **Portal UI:** sticky navy header (mark-only logo, bell, name, Admin Editor pill for superadmins,
  ...
  **Payments** pill (history newest first, status pill, Copy pay link, **Start New Payment**;
  ...
  rules) and Email Templates (two seeded rows; sections Client Payments / COI Payouts / Other); the
  other four are the placeholder panels named in OWED. sessionStorage:
  `wigActiveTab`, `wigCoiSection`, `wigSelectedCoi`, `wigCoiFeatureTab`, `wigAutomationSection`,
  `wigAccountingSection`, `wigSelectedMothership`, `wigCoiReturnTo` — cleared on sign-in,
  sign-out-to-welcome and nav; the last two survive the mothership→COI round trip, re-written after.
- **Standing UI rules (permanent — Jake):** (1) the hero is flush at the top and the "← Back to …"
  ...
  it is a shortcut — rows that navigate on click keep plain names (`NameLink`); (3) interaction
  mechanics copy the VFO portal exactly where one exists, hover timing included.
- **Backend (v: 2026-09-02):** `iag-admin-api` **v17**, ACTIVE, `verify_jwt: false` (custom auth, in
  the function). Deno 2. Project ref `gqznnyccridnpipjipeq`. 51 `.ts` files, ~174 KB. Post-deploy
  smoke: both public pay handlers 200 `state: "invalid"` on a junk token, `start_client_payment` 401.
- **Actions (33, v: 2026-09-02):** `admin_login` (direct in `index.ts`); public pre-auth
  ...
  `start_client_payment`, `load_client_payments`, `load_strategies`, `save_strategy`,
  `load_email_templates`, `save_email_template`, `create_test_checkout`, `admin_test_draft`.
  `*_admin*` / `load_admins` are **superadmin-only** (an `auth.isSuperadmin` 403 as the handler's
  first line — the auth gate proves a session, never a rank).
- **Database (v: 2026-09-02):** 12 public tables — `admins`, `admin_sessions`, `login_attempts`,
  ...
  never payload-writable — `update_coi` must touch neither. `email_templates` holds TWO rows,
  draft and active: `COI_PAYOUT`/`coi_connect_setup` and `CLIENT_PAYMENT`/`client_payment_request`.
...
- **Revenue share (v: 2026-09-02):** `motherships` (number PK, ERT = 1) is the firm a COI sits
  ...
  `created_by`, then `stripe_customer_id` + `checkout_token`, then `payment_email_sent_at`;
  everything from `payment_status` on is unwritten (Phase D books it from the webhook; the waterfall
  columns are Phase F's). The request form's preview is DISPLAY ONLY — nothing it computes is sent.
- **Migrations:** 14, applied via MCP `apply_migration` AND committed under `supabase/migrations/`.
...
- **Auth:** custom sessions, 8h, `login_type` `"admin"`. Passcodes PBKDF2 210k, salted, min length 8
  ...
  `fabot@wealthig.com` (`constants/superadmin.ts`) outranks `admins.is_superadmin` and can never be
  deleted. `update_passcode` targets the SESSION's admin only and revokes their OTHER sessions. New
  admins get a NULL passcode plus a 14-day single-use `/set-password` link; no self-service reset
  (PARKED). `middleware/auth.ts` splits **401 from 500**: a bad/expired/missing credential is 401, a
  FAILED DB read is 500, because the frontend signs out on any 401 (#12). Every 401 logs its reason,
  never its token.
- **Stripe:** the IAG Portal's own account, entirely separate from VFO. Test-mode AND live-mode
  ...
  reads status live and never stores it. Client-fee Checkout is ACH only (`us_bank_account`, instant
  verification). Flows: `coi-connect-setup.md`, `client-payment-request.md`.
- **Gmail:** Google Cloud project "IAG Portal" in the wealthig.com org. Consent screen INTERNAL,
  ...
  URI = OAuth Playground), scope `gmail.compose`. **Drafts only — no send path exists.** Two draft
  flows: the COI setup email and the client payment request; `/set-password` links go by hand.
...
- **`email_templates` has TWO rows** (v: 2026-09-02) — `coi_connect_setup`, `client_payment_request`.
  Every other pipeline's subjects and bodies still need Jake's approval in chat before seeding.
...
- **No "resend payment email" action** (v: 2026-09-02). A payment whose Gmail draft failed shows
  "Email not sent" and cannot be re-emailed — the admin copies the pay link or raises a new request,
  and pressing Start New Payment again is a SECOND payment. `/pay?done=1` is likewise a plain
  `AuthShell` card, standing in until the WIG-branded payment-success landing page lands.
- **Phase C frontend is NOT deployed** (v: 2026-09-02): `/pay`, the Payments tab and the form exist only
  in the worktree, so emailed links 404 until `npm run deploy`. `/pay`'s left panel still says "team portal".
- **The ERT-percentage BASE is not stated in the words** (v: 2026-09-02). The seeded LEOS `explainer`
  step 3 and the Tax Strategies panel's step-3 card give the 10%/5% without saying they come off the
  fee AFTER the hard costs. Reword both, and Phase F must implement that rule server-side.
...
- **Stripe Connect platform review is still PENDING.** Sandbox is fully proven, so the build is NOT
  blocked; only LIVE mode is — real COI onboarding needs the review AND the `STRIPE_MODE` flip below.
- **The `STRIPE_MODE` flip to live is a deliberate later decision**, not a chore — a constant in
  `utils/stripe.ts`, live webhook endpoint already registered, and it goes live the moment it deploys.
...
- **Deploys:** backend via `scripts/deploy-function.sh` (Management API upload, token read from
  `.mcp.json`) — in PowerShell `& "$HOME\scoop\apps\git\current\usr\bin\bash.exe" scripts/deploy-function.sh`,
  in Git Bash a bare `bash`. The MCP tool no longer fits; the CLI is forbidden. (GOTCHAS #5/#13/#15)
